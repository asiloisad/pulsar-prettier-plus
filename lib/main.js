          ...
          try {
            const results = await formatProject(projectPath, {
              onProgress({ current, total, filePath }) {
                const el = notification.getElement
                  ? notification.getElement()
                  : null;
                const detail = el
                  ? el.querySelector(".detail-content")
                  : null;
                if (detail) {
                  detail.textContent = `[${projectName}] ${current}/${total}: ${path.basename(filePath)}`;
                }
              },
            });
            ...
            atom.notifications.addError(
              `prettier-plus: Error formatting "${projectName}"`,
              { dismissable: true, detail: err.stack }
            );
          }
        }

...
const errorLine = (error) =>
  error.loc.start ? error.loc.start.line : error.loc.line;
...
const buildExcerpt = (error) =>
  _.get("[1]", /(.*)\s\(\d+:\d+\).*/.exec(error.message));
...
const isFilePathPresent = _.flow(
  _.get("editor"),
  getCurrentFilePath,
  _.negate(_.isNil)
);
...
const handleError = _.flow(
  _.cond([
    ...
    [
      isUndefinedError,
      (args) => console.error("Prettier encountered an error:", args.error),
    ],

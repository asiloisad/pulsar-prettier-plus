...
const displayImproperPrettierVersionError = _.once(() =>
  addErrorNotification(
    `Your Prettier version is not compatible with prettier-plus. Prettier must be >= ${MINIMUM_PRETTIER_VERSION_FOR_ATOM_PRETTIER_ATOM_TO_WORK}`,
    {
      dismissable: true,
      detail:
...
const isGetFileInfoDefined = (prettier) =>
  !!_.get("getFileInfo.sync", prettier);

const isPrettierProperVersion = _.flow(
  getPrettierInstance,
